...
<template>
  ...
  <div class="pt-page pt-page-current pt-page-controller search-pager">
    <PageHeader ref="pageHeader" :labels="labels" pid="vfte018" version="1.0.0" showLanguage="true" @language-changed="changeLanguage" :multiLanguages="multiLanguages" :build="buildVersion" :visible="displayPageHeader" />
...
export default {
    ...
    this.$nextTick(async () => {
      ...
      startApplication("vfte018",(data) => {
        ...
        if(data.type=="language") {
          ...
          loadAndMergeLabel("vfte018", (success) => {
            if (success) {
  ...
  methods: {
    ...
    dataSelected(item,action) {
      ...
      if("edit"==action) {
        this.$refs.entryForm.retrieveRecord({tenantid: item.tenantid});
      } else if("delete"==action) {
        this.$refs.entryForm.startDeleteRecord({tenantname: item.tenantname, tenantid: item.tenantid});
      }
    },
    dataInsert(filters) {
      //listen action from search form
